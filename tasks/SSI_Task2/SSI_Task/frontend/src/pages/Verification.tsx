...
export const Verification: React.FC = () => {
  const {
    ...
    bobAgent,
    refreshConnections,
    acceptProofRequest, // ADD THIS FROM CONTEXT
  ...
  const handleAcceptAndPresentProof = async (proofId: string) => {
    ...
    try {
      ...
      await acceptProofRequest(proofId); // NOW THIS WILL WORK
  ...
  const handleViewAttributes = async (proofId: string) => {
    try {
      ...
      const mappedAttrs = Object.values(attrs).map((item: any) => ({
        name: item.name,
        value: item.value,
      }));
